...
public class WebController extends BaseWebController {
    ...
    @Autowired
    private ServerLoginServiceFeign loginServiceFeign;
    ...
    public String doLogin(HttpServletRequest request,
        ...
        UserLoginServerInpDTO userLoginServerInpDTO = new UserLoginServerInpDTO();
        userLoginServerInpDTO.setMobile(username);
        userLoginServerInpDTO.setPassword(password);
        BaseResponse<UserOutDTO> login = loginServiceFeign.login(userLoginServerInpDTO);
        //登录不成功
        if (!isSuccess(login)){
...
//        }
        ...
        // 1、make xxl-sso user
        UserOutDTO userOutDTO = login.getData();
        if (userOutDTO == null){
        ...
        xxlUser.setUserid(String.valueOf(userOutDTO.getUserId()));
